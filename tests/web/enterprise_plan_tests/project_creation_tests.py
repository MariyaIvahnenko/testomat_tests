...
def test_new_project_creation_and_test_popup(logged_app: Application):
    ...
    logged_app.test_for_suite_popup.wait_for_visible().select_first_suite()

    test_name = Faker().sentence()
    logged_app.test_modal.wait_for_loaded("test").set_title(test_name).save()
...
def test_open_project_and_create_test_suite_from_side_bar(logged_app: Application, api_client: TestomatClient):
    ...
    test_name = Faker().sentence()
    logged_app.test_modal.wait_for_loaded("Suite").set_title(test_name).save()
